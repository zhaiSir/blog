...
/**点击置顶*/
$("#fileLearningForm").on("click",".update-top",function () {
    var me = $(this);
    //获取id
    ...
    $.ajax({
        url:rootPath+"/filelearning/updateTop",
        type:"get",
        dataType:"json",
        ...
        success:function (edc) {
            if(edc.state == -1){
                ...
                window.open(rootPath+"/filelearning/list","_self");
            }
        }
    });
});
...
/**发布*/
$("#fileLearningForm").on("click","#release",function () {
    ...
    if (confirm("是否发布")){
        ...
        $.ajax({
            type:"post",
            data:"fileLearningId="+id,
            url:rootPath+"/filelearning/release",
            dataType:"json",
...
/**审核*/
$("#fileLearningForm").on("click","#audit",function () {
    var me = $(this);
    var id = me.closest("tr").data("uniqueid");
        ...
        $.ajax({
            type:"post",
            data:"fileLearningId="+id,
            url:rootPath+"/filelearning/ableAudit",
            dataType:"json",
            success:function (res) {

                if (res == 1){
                    window.open(rootPath+"/filelearning/get?fileLearningId="+id+"&getIs=audit");
                }else {
                    layer.alert("不满足审核条件！");
                }
...
$(".bt-box").on("click","#data-disable",function (e) {
    ...
    if (selectIds.length == 0){
        ...
        if (confirm("是否停用")){
            ...
            $.ajax({
                type:"post",
                data:"fileLearningId="+selectIds,
                url:rootPath+"/filelearning/disenable",
                dataType:"json",
                success:function (resultJson) {
                    if (resultJson.state == 0){
                        layer.alert(resultJson.message);
    ...
    if (selectIds.length == 0){
        ...
        if (confirm("是否启用")){
            $.ajax({
                type:"post",
                data:"fileLearningId="+selectIds,
                url:rootPath+"/filelearning/enable",
                dataType:"json",
                success:function (resultJson) {
                    if (resultJson.state == 0){
                        layer.alert(resultJson.message);
...
/**跳转修改页面*/
$("#fileLearningForm").on("click",".update-dialog",function () {
    var me = $(this);
    var href = rootPath + "/filelearning/update";
    ...
    $.ajax({
        type:"post",
        data:"fileLearningId="+id,
        url:rootPath+"/filelearning/isupdate",
        dataType:"json",
        success:function (resultJson) {
            if (resultJson.state == 0){
                layer.alert(resultJson.message);
...
//点击进入详情页面
$("#fileLearningForm").on("click",".get-dialog",function () {
    var me = $(this);
    var href = rootPath + "/filelearning/get";
    var btTable = $("#"+WELKIN.getComponentTableId(this));
    var uniqueId = btTable.bootstrapTable('getOptions').uniqueId;
    var id = me.closest("tr").data("uniqueid");
    var paramStr = uniqueId+"="+id;
...
function addNewOnclick() {
    window.open(rootPath+"/filelearning/add");
}

//点击名单查看按钮弹出新的页面
$("#fileLearningForm").on("click",".get-whitelist",function () {
    var me = $(this);
    var id = me.closest("tr").data("uniqueid");
    window.open(rootPath + "/viewwhitelist/list?relationId="+id+"&category=filelearning");
});


//机构选择
$("#fileLearningForm").on("click",".get-org",function () {
    var me = $(this);
